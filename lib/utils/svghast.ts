...
const svghast = (svgIcon: SvgIconType) => {
  const SvgIcon = SvgIcons[svgIcon]

  const path = h('path')
  const pathData = path.data || (path.data = {})
  pathData.hName = 'path'
  pathData.hProperties = h('path', SvgIcon.path.props).properties

  const icon = h('svg')
  const iconData = icon.data || (icon.data = {})
  iconData.hName = 'svg'
  iconData.hProperties = h('svg', SvgIcon.props).properties
  icon.children = [path]

  return icon
}

export default svghast
